...
#EOF
hname=$(hostname)
ladd=$(ip a s $(ip a |awk '/: e/{gsub(/:/,"");print $2}')|awk '/inet /{gsub(/\/.*/,"");print $2}')
lhname=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
eip=$(curl -s icanhazip.com)
exname=$(getent hosts $(curl -s icanhazip.com) | awk '{print $2}')
ripadd=$(ip r | awk '/via /{gsub(/\/.*/,"");print $3}')
rname=$(getent hosts $ripadd | awk '{print $2}')
netnum=$(ip r | awk '/src /{gsub(/\/.*/,"");print $1}')
